...
from modulestf.const import OUTPUT_DIR, WORK_DIR, FINAL_DIR
...
def convert_url(view_url):
    api_url = view_url.replace("https://app.cloudcraft.co/view/", "https://app.cloudcraft.co/api/blueprint/")
    fixture_json = {
        "body": "",
...
if __name__ == "__main__":
    ...
    parser.add_argument("-u", "--url", help="Cloudcraft View URL", required=True)
    ...
    fixture_json = convert_url(args.url)
    ...
    print("\n\n\n====================\n")
    print(f"Input Diagram URL: {args.url}")
    print(f"Output directory: {pathlib.Path(OUTPUT_DIR + '/' + WORK_DIR + '/' + FINAL_DIR).absolute()}")
